...
// Helper to create conditional schemas based on account type
const createConditionalSchema = (accountTypes: ('personal' | 'business' | 'merchant')[]) => {
  return z.preprocess((val) => val, z.any())
    .superRefine((val, ctx) => {
      // Safe access to parent data to avoid errors when ctx.parent is undefined
      const formData = ctx.parent?.parent || {};
      ...
      // Skip validation if account type is not in the list
      if (!accountType || !accountTypes.includes(accountType)) return;
...
export const signupFormSchema = z.object({
  ...
  business_name: z.string().min(2, 'Professional name must be at least 2 characters')
    .optional()
  ...
  bank_account_type: z.string().min(2, 'Account type must be at least 2 characters')
    .optional()
    .and(createConditionalSchema(['merchant'])),

  account_currency: z.string().min(2, 'Currency must be at least 2 characters')
    .optional()
    .and(createConditionalSchema(['merchant'])),

  bank_account_number: z.string().min(4, 'Account number must be at least 4 characters')
    .optional()
    .and(createConditionalSchema(['merchant'])),

  bank_branch: z.string().min(2, 'Branch must be at least 2 characters')
    .optional()
    .and(createConditionalSchema(['merchant'])),

  id_image_url: z.string()
    .optional()
    .or(z.literal(''))
    .or(z.instanceof(File)) // Support File objects that might come from the file input
    .and(createConditionalSchema(['merchant'])),
