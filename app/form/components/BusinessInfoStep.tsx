...
export const BusinessInfoStep: React.FC<StepProps> = ({ currentStep, setCurrentStep }) => {
  const { register, formState: { errors }, watch, setValue, clearErrors } = useFormContext<SignupFormData>();
  ...
  return (
    <div className="bg-white rounded-lg">
      <h3 className="text-xl font-semibold text-gray-800 mb-6">
        ...
        {!isBusinessInfoRequired && <span className="text-sm font-normal text-gray-500 ml-2">(Optional for Merchants)</span>}
      </h3>
      ...
      <div className="form-group">
        <label htmlFor="business_name" className="form-label">
          ...
          {!isBusinessInfoRequired && <span className="text-gray-400 text-xs ml-2">(Optional)</span>}
        </label>
        ...
        {errors.business_name && <p className="form-error" role="alert">{errors.business_name.message}</p>}
      </div>
      ...
      <div className="form-group">
        ...
        <div className="relative">
          ...
          <textarea 
            ...
            className="form-input input-with-icon" 
            rows={3}
            placeholder={isBusinessInfoRequired ? "Enter your business address" : "Optional for merchants"}
            aria-required={isBusinessInfoRequired}
            aria-invalid={errors.business_address ? "true" : "false"}
          />
        </div>
        {errors.business_address && <p className="form-error" role="alert">{errors.business_address.message}</p>}
      </div>
